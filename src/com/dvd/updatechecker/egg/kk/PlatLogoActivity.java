...
public class PlatLogoActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String kkClicks = pref.getString("kk_clicks", null);
		int kkSize = pref.getInt("kk_letter_size", 300);
		int kkTextSize = pref.getInt("kk_text_size", 30);
		String leMode = pref.getString("kk_sysui", null);

		if (leMode.equals(getString(R.string.none)))
			getWindow().setFlags(WindowManager.LayoutParams.FLAG_FULLSCREEN,
					WindowManager.LayoutParams.FLAG_FULLSCREEN);

		try {

			if (kkClicks != null && kkClicks.length() > 0
					&& kkClicks.matches("\\d*")
					&& Integer.parseInt(kkClicks) > 0) {

				MAX_CLICKS = Integer.parseInt(kkClicks);
		...
		try {
			...
			if (String.valueOf(kkSize) != null && kkSize > 0
					&& String.valueOf(kkSize).matches("\\d*")
					&& String.valueOf(kkSize).length() > 0) {
		...
		try {
			...
			if (String.valueOf(kkTextSize) != null && kkTextSize > 0
					&& String.valueOf(kkTextSize).matches("\\d*")
					&& String.valueOf(kkTextSize).length() > 0) {
		...
		final LayoutParams lapar = new LayoutParams(LayoutParams.WRAP_CONTENT,
				LayoutParams.WRAP_CONTENT);
		lapar.gravity = Gravity.BOTTOM | Gravity.CENTER_HORIZONTAL;
		if (leMode.equals(getString(R.string.none)))
			lapar.bottomMargin = 10 * p;
		else
			lapar.bottomMargin = 15 * p;
		...
		mContent.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (interpol == "Accelerate") {
					...
					whichinterp = new AnticipateOvershootInterpolator(
							(float) 0.9);
					dur = 1500;
		...
		if (leMode.equals(getString(R.string.translucent))) {
			getWindow().setFlags(
		...
		if (leMode.equals(getString(R.string.immerge))) {
